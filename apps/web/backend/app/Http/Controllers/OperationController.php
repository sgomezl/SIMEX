  ...
  public function index(Request $request)
  {
    try {
      ...
      ->with([
          ...
          'stateHistory',
          'incoterm' => function($q) { $q->with('incotermType', 'trackingStep'); }
      ]);
  ...
  public function show($id)
  {
    try {
      $operation = Operations::with([
        'createUser' => function($q) { $q->with('role', 'company'); },
        'importer' => function($q) { $q->with('companyType', 'region', 'city'); },
        'incoterm' => function($q) { $q->with('incotermType', 'trackingStep'); },
        'exportator' => function($q) { $q->with('companyType', 'region', 'city'); },
        'operationUser' => function($q) { $q->with('role', 'company'); },
        'salesUser' => function($q) { $q->with('role', 'company'); },
        'documentUser' => function($q) { $q->with('role', 'company'); },
        'containerType',
        'packageType',
        'packageSubType',
        'buyer' => function($q) { $q->with('companyType', 'region', 'city'); },
        'seller' => function($q) { $q->with('companyType', 'region', 'city'); },
        'naviera' => function($q) { $q->with('companyType', 'region', 'city'); },
        'originPort' => function($q) { $q->with('city.country', 'city.region'); },
        'destinationPort' => function($q) { $q->with('city.country', 'city.region'); }
      ])->findOrFail($id);
